import string
rooms = """nzydfxpc-rclop-qwzhpc-qtylyntyr-769[oshgk]
...
def is_real(room):
    parts = room[:-1].split('[')
    checksum =  parts[1]
    name_sector = parts[0].split('-')
    sector = int(name_sector[-1])
    name = ''.join(name_sector[:-1])
    data ={}
    for l in name:
        if l not in data:
            data[l] = 0
    ...
    return checksum == "".join(a[0] for a in items[:5])

room = "qzmt-zixmtkozy-ivhz-343"
def get_real_name(room):
    room = room.split('[')[0]
    splitat = room.rindex('-')
    room_id = int(room[splitat+1:])
    encrypted_name = room[:splitat]
    name = ""
    for letter in encrypted_name:
        ind = string.ascii_lowercase.find(letter)
        if ind == -1:
            name = name +" "
        else:
            name = name + string.ascii_lowercase[(ind+ room_id)%len(string.ascii_lowercase)]
    return name
#print(get_real_name(room))
#print(string.ascii_lowercase)



print([room for room in rooms if is_real(room) and "northpole object storage" == get_real_name(room)])
