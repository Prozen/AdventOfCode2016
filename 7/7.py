data = """dnwtsgywerfamfv[gwrhdujbiowtcirq]bjbhmuxdcasenlctwgh
...
def isTLS(ip):
    return len(ip) > 3 and ((ip[0]==ip[3] and ip[1]== ip[2] and ip[0]!= ip[1]) or isTLS(ip[1:]))
...
for ip in data:
    good, bad = split(ip)
    if any(isTLS(x) for x in good) and not any(isTLS(x) for x in bad):
        count+=1
print(count)
