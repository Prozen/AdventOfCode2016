instructions ="""bot 88 gives low to bot 51 and high to bot 42
...
def give( robot, coin):
    if robot not in state:
        state[robot] = []
    state[robot].append(coin)

for instruction in instructions:
    if "goes to" in instruction:
        splitted =  instruction[6:].split(" goes to ")
        give(splitted[1], int(splitted[0]))

while any(len(x[1]) > 1 for x in state.items() if "bot" in x[0]):
    for instruction in instructions:
        if "goes to" in instruction:
            pass
        else:
            splitted = instruction.split(" gives low to ")
            low_high = splitted[1].split(" and high to ")
            if splitted[0] in state and len(state[splitted[0]])==2:
                if min(state[splitted[0]]) == 17 and max(state[splitted[0]]) ==61:
                    print(splitted[0], state[splitted[0]])
                give(low_high[0],min(state[splitted[0]]))
                give(low_high[1],max(state[splitted[0]]))
                state[splitted[0]]=[]
print(state["output 1"], state["output 2"], state["output 0"])
